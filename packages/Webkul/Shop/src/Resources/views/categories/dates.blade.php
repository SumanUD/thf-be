...
<body class="thf-dark-theme">
    ...
    @pushOnce('scripts')
    <script type="text/x-template" id="v-dates-products-template">
    ...
    <script type="module">
        app.component('v-dates-products', {
            template: '#v-dates-products-template',
            data() {
                return {
                    products: []
                }
            ...
            methods: {
                getProducts() {
                    this.$axios.get("{{ route('shop.api.products.index', ['category_id' => 4]) }}")
                        .then(response => {
